...
class ProcessSentences:
    ...
    def hasNumbers(self):
        ...
        for sentence in self.getTokenizedSentences():
    ...
    def nounFeatureScoring(self):
        sentences = self.getTokenizedSentences()
        sentenceNounScore = dict()
        for sentence in sentences:
            textBlob = TextBlob(sentence)
            tags = Counter(convertListToString(textBlob.tags[1]))
            if "NNP" in tags.keys():
                sentenceNounScore[sentence] += tags["NNP"]
            if "NNPS" in tags.keys():
                sentenceNounScore[sentence] += tags["NNP"]
...
sample_text = "मेरे पास दो कुत्ते हैं, और एक बिल्ली है"
t = ProcessSentences(sample_text)
print(t.hasNumbers())